...
let macroRoll = `{{roll=[[1d20cs>${config.weapon.critRange} + ${config.base.bab}[BAB] + ${config.base.strBonus}[Strength] + ${config.macro.attackBonus}[Buff]]]}}{{critconfirm=[[1d20 + ${config.base.bab}[BAB] + ${config.base.strBonus}[Strength] + ${config.macro.attackBonus}[Buff] ?{AttackMod|0} ]]}}{{atkvs=Melee+STR vs AC}}`;
...
function calculateMacroMulti(actionName) {
  config.calculateAttack(actionName);
  const macroBase = macroPrefix + macroRoll + macroDamage;
